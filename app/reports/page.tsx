...
export default async function Reports({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <>
    ...
    <div className="card" style={{marginTop:18}}><div className="sectionHeader"><div><h2><BarChart3 size={17} style={{verticalAlign:'-3px',marginRight:7}}/>Rekonsiliasi Eliminasi</h2><div className="muted">Pasangan akun eliminasi yang memengaruhi laporan periode ini.</div></div></div><div className="tableWrap"><table className="table"><thead><tr><th>Pasangan Akun</th><th className="numHead">Saldo A</th><th className="numHead">Saldo B</th><th className="numHead">Nilai Eliminasi</th></tr></thead><tbody>{r.eliminationRows.map(e=><tr key={e.id}><td>{e.label}</td><td className="numCell">{fmt(e.source)}</td><td className="numCell">{fmt(e.target)}</td><td className="numCell reportTotal">{fmt(e.amount)}</td></tr>)}</tbody></table></div>{!r.eliminationRows.length?<div className="empty">Belum ada mapping eliminasi.</div>:null}</div>
  </>
}
