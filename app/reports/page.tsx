...
export default async function Reports({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <>
    ...
    <div className="card" style={{marginTop:18}}><div className="sectionHeader"><div><h2><BarChart3 size={17} style={{verticalAlign:'-3px',marginRight:7}}/>Jurnal Eliminasi</h2><div className="muted">Setiap mapping disimpan sebagai jurnal eliminasi dua sisi dengan total debit = kredit. Jurnal hanya memengaruhi laporan sesuai tanggal eliminasi yang ditentukan.</div></div></div><div className="tableWrap"><table className="table"><thead><tr><th>Tanggal</th><th>Pasangan Akun</th><th>Status</th><th className="numHead">Debit A</th><th className="numHead">Kredit A</th><th className="numHead">Debit B</th><th className="numHead">Kredit B</th><th className="numHead">Nilai Eliminasi</th></tr></thead><tbody>{r.eliminationRows.map(e=><tr key={e.id}><td>{e.date.toLocaleDateString('id-ID')}</td><td>{e.label}</td><td>{e.status==='BALANCED'?'Balance':'Tidak ada jurnal tersimpan'}</td><td className="numCell">{fmt(e.sourceDebit)}</td><td className="numCell">{fmt(e.sourceCredit)}</td><td className="numCell">{fmt(e.targetDebit)}</td><td className="numCell">{fmt(e.targetCredit)}</td><td className="numCell reportTotal">{fmt(e.amount)}</td></tr>)}</tbody></table></div>{!r.eliminationRows.length?<div className="empty">Belum ada mapping eliminasi.</div>:null}</div>
  </>
}
