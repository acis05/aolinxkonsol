...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  let c=await prisma.company.findUniqueOrThrow({where:{id}})
  if(!c.accurateDbId) throw new Error('Company belum terhubung ke database Accurate')
  const opened=await openDatabase(c.accurateDbId)
